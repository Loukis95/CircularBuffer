...
int main(int argc, char** argv)
{
	...
	try
	{
		try
		{
			std::cout << "Attempting to retrieve the front element when the buffer is empty" << std::endl;
			c = buffer.front();
			std::cerr << "Calling front() on an empty buffer shall fail" << std::endl;
			return 1;
		}
		...
		std::cout << "Attempting to retrieve the front element" << std::endl;
		c = buffer.front();
		assert(c == 'a');
	}
